...
def fusionTree(tree1, f, tree2):
    ...
    values = np.zeros((dic['node_count'], dic['values'].shape[
                      1], dic['values'].shape[2]))
    dic['values'] = values
...
def cut_into_leaves_tree(tree, nodes):
    ...
    for i in range(dic['nodes']['left_child'].size):
        if (dic['nodes']['left_child'][i] != -1):
            dic['nodes']['left_child'][i] = ind.index(
                dic['nodes']['left_child'][i])

    for i in range(dic['nodes']['right_child'].size):
        if (dic['nodes']['right_child'][i] != -1):
            dic['nodes']['right_child'][i] = ind.index(
                dic['nodes']['right_child'][i])
...
def SER(dTree, X_target, y_target):
    ...
    for f in leaves:
        ...
        if ind.size != 0:
            ...
            # min_impurity_split deprecated in sklearn 0.18 -> use
...
def all_splits(X, ind_feats, min_delta=10e-7):
    ...
    for c in range(x_sorted.shape[1]):
        ...
        inds[c] = np.where(var > min_delta *
                           (max(x_sorted[:, c]) - min(x_sorted[:, c])))[0]
        ...
        for th in f:
            ...
            for i, and_block in enumerate(measure_func):
                ...
                for j, func in enumerate(and_block):
                    and_block_score[i][j] = func(
                        Q_source_parent, [Q_target_l, Q_target_r])
...
if __name__ == "__main__":
    ...
    rf = (skl_ens.RandomForestClassifier(
        n_estimators=50, max_depth=10, oob_score=True))
    rf.fit(X[train], y[train])
    dTree = rf.estimators_[0]

    print('Arbre initial appr. sur ' + str(train.size) +
