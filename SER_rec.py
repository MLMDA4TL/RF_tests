...
def error(tree, node):
    if node == -1:
        ...
        if tree.feature[node] == -2:
            ...
            nr = tree.weighted_n_node_samples[tree.children_right[node]]
            nl = tree.weighted_n_node_samples[tree.children_left[node]]
...
def cut_from_left_right(dTree, node, bool_left_right):
    ...
    if bool_left_right == 1:
        ...
        node_to_rem = [node] + \
            sub_nodes(dTree.tree_, dTree.tree_.children_right[node])

    elif bool_left_right == -1:
        repl_node = dTree.tree_.children_right[node]
        node_to_rem = [node] + \
            sub_nodes(dTree.tree_, dTree.tree_.children_left[node])
...
def SER(node, dTree, X_target_node, y_target_node):
    ...
    new_node_left = SER(dTree.tree_.children_left[
                        node], dTree, X_target_node_left, y_target_node_left)
    dic = dTree.tree_.__getstate__().copy()
    node, b = find_parent(dic, new_node_left)
    new_node_right = SER(dTree.tree_.children_right[
                         node], dTree, X_target_node_right, y_target_node_right)
    dic = dTree.tree_.__getstate__().copy()
...
def SER_RF(random_forest, X_target, y_target):
    ...
    for i, dtree in enumerate(rf_ser.estimators_):
        print("tree n° ", i)
        SER(0, rf_ser.estimators_[i], X_target, y_target)
    return rf_ser
